...
import { User, LockKeyhole, Mail, Phone, MapPin, Pencil, Upload, X } from "lucide-react";
      ...
      <div className="flex">
          ...
          <div className="w-full px-10 py-8 flex flex-col items-end">
            <div className="w-full max-w-70">
              ...
              <div className="flex items-center gap-4 mb-10">
                <div className="cursor-pointer" onClick={() => setShowImageOverlay(true)}>
                  <Image
                    src={user.avatarUrl || "/image/default.png"}
                    alt="avatar"
                    width={56}
                    height={56}
                    className="rounded-full aspect-square object-cover border border-border hover:brightness-90 transition"
                  />
                </div>
              ...
              <div className="space-y-1">
                <button
                  ...
                  ${tab === "profile" ? "bg-accent text-accent-foreground" : "hover:bg-accent text-muted-foreground hover:text-foreground"}`}
                >
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-3 py-3 rounded-lg transition
                  ${tab === "password" ? "bg-accent text-accent-foreground" : "hover:bg-accent text-muted-foreground hover:text-foreground"}`}
                >
        ...
        <main className="flex-1 flex justify-start">
          <div className="w-full max-w-225 px-2 md:px-14 py-10">
            ...
            <div className="pt-3">
              {tab === "profile" ? (
                <ProfileTab user={user} setUser={setUser} onAvatarClick={() => setShowImageOverlay(true)} />
              ) : (
      ...
      {showImageOverlay && (
        ...
        >
          <div className="relative max-w-[90vw] max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              onClick={() => setShowImageOverlay(false)}
              className="absolute -top-5 -right-5 bg-background text-foreground rounded-full p-3 shadow-xl hover:bg-muted transition-all"
            >
              <X size={28} />
            </button>
...
function ProfileTab({ user, setUser, onAvatarClick }: { user: any; setUser: any; onAvatarClick: () => void }) {
      ...
      <div className="flex flex-col items-center md:flex-row gap-6 pb-4 border-b border-border">
        <div className="relative cursor-pointer" onClick={onAvatarClick}>
          <Image
            ...
            width={110}
            height={110}
            className="rounded-full object-cover aspect-square border-4 border-background shadow-md hover:brightness-90 transition"
          />
        </div>
